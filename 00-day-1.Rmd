...
get your working envrionment set up.
...
5. Download the [code handout](https://southampton-rsg.github.io/data-analysis-and-visualisation/code-handout.R), place it in your working
...
* The other people . You might also be interested in organizing regular meetings following
...
Now that we have a working envronment set up we can start on the basics of R. 
...
An _object_ is programming speek for a thing with known properties. Lets consider 
an example that we would all be fimmilar with before jumping into some R. 

I tell you that I have a _car_, in this case the car is the _object_.
Without any further information you instintivly know that the _object_ called _car_
...
model(_car_) I can tell you 'Corsa'. However, if you were to ask me breed(_car_) this would be a
...
> Including but not limeted to: Species, Age, Colour, Name, Sex, Owner... 
> ```

In this tutorial we will use the predefined R objects for vectors and numerical values.
We will see that these are much simpler than the linguisitc examples above, and we learn the
sort of _attributes_ we expect them to have and how to query an object to tell us its 
set of _attributes_.
...
when using R studio to understand what objects you have. This tab is great for small variables, 
...
that can be managed `by hand`. With R will will be able to load large sets of data and create 
readable, reliable, and reproducable data analysis scripts.
...
loaded, we can check the envronment pane in RStudio (or if you dont mind a *large* 
amount of data bing printed type `surveys`). 
...
```{r, results=TRUE, purl=FALSE}
missing_dates_vec <- date_char_vec[is.na(date_vec)]
...
### Note 
> For completeness sake it is worth noting that the above could be acchived in one line.
...
> This way the r envronment is kept cleaner however it is more difficult to unpick where 
> errors have occured. 
...
skillset.
Becoming fimmilar with the basics we have covered today and the inspection tools e.g. 
